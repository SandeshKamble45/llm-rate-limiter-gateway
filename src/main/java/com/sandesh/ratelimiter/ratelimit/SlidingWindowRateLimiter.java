...
public class SlidingWindowRateLimiter {
        ...
        public SlidingWindowStatus getStatus(String tenantKey) {

                String redisKey = "ratelimit:sw:" + tenantKey;

                long nowMilliseconds = System.currentTimeMillis();

                long windowStart = nowMilliseconds
                                - (windowSeconds * 1000);

                Set<Object> requestIds = redisTemplate.opsForZSet()
                                .rangeByScore(
                                                redisKey,
                                                windowStart,
                                                Double.POSITIVE_INFINITY);

                long currentRequests = requestIds == null
                                ? 0
                                : requestIds.size();
                ...
                return new SlidingWindowStatus(
                                currentRequests,
                                maxRequests,
                                windowSeconds,
                                remainingRequests);
